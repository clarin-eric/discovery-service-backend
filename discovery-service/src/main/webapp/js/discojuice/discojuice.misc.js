...
DiscoJuice.Utils = {
    ...
    "searchMatch": function (item, term) {
        //search in all titles for a term match
        for (var i = 0; i < item.titles.length; i++) {
            var title = item.titles[i].value;
            if (title.toLowerCase().search(term.toLowerCase()) !== -1)
                return true;
        }

        if (item.descr && item.descr.toLowerCase().search(term.toLowerCase()) !== -1)
            return true;
        var key, i, keyword;
        ...
        if (item.keywords) {
            for (key in item.keywords) {
                ...
                for (i = 0; i < keyword.length; i++) {
                    if (keyword[i].toLowerCase().search(term.toLowerCase()) !== -1)
                        return keyword[i];
                }
            }
        }
